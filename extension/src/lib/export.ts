  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    if (step.screenshotId) {
      ...
      if (blob) {
        ...
        img = `<img src="${dataUrl}" alt="Step ${i + 1} screenshot" loading="lazy" />`;
      }
    }
...
/** Build the HTML guide and trigger a browser download. Used from both the
 * editor and the side panel — the click+revoke dance is identical, so it lives
 * here to avoid drift. */
export async function downloadHtmlGuide(
  ...
  try {
    ...
    a.href = url;
    a.download = `${recording.name.replace(/[^\w\d-]+/g, '-').slice(0, 80)}.html`;
